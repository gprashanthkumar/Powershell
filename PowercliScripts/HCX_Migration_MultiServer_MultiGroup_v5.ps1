...
$HCXVMS = Import-CSV "C:\Temp\Wave9\Wave9-MobilityGroup_full.csv"
...
$MobilityGroupName = "Wave3_5tMay_" + $(Get-Date).ToString("yyyyMMdd_HHmmss_fff") ;
$startTime = [datetime]::ParseExact("09-06-2025", "dd-MM-yyyy", $null)
$startTime = $startTime.AddHours(3)
Write-Output "--- Frist vm start time --" 
$startTime

#$endTime = $startTime.AddMinutes(30)
$endTime = [datetime]::ParseExact("13-06-2025", "dd-MM-yyyy", $null)
...
ForEach ($HCXVM in $HCXVMS) {
    ...
    if ($vm) {
        ...
        if ($vm.PowerState  -eq  'PoweredOn') {
                         ...
                         #$vm.Site.Name
                $MobilityGroupName = "WAVE9_DE_MIG_RAV_1061_05" # + "-1_" + $vm.Network[0].Name.Replace("(","_").Replace(")","").Replace("-","_").Replace(" ", "");
                ...
                $destSite = Get-HCXSite -Destination -Name $HCXVM.DEST_SITE
                 Write-Output " Destination Network"
                 $destSite
                ...
                #$destCompute
                  ...
                  $NewMigration = New-HCXMigration   -VM $vm -NetworkMapping $NetworkMapping  -MigrationType RAV -SourceSite $srcSite -DestinationSite $destSite  -TargetComputeContainer $destCompute -TargetDatastore $destDS -DiskProvisionType Thin -MobilityGroupMigration -RetainMac $True -EnableSeedCheckpoint $true -RemoveSnapshots $True -RemoveISOs $true -ScheduleStartTime $startTime -ScheduleEndTime $endTime
