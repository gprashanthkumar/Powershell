...
$SleepSeconds = 2;
$Output = "";
$WaveOutputFilPath = "C:\Temp\$Wave\VMMigrated_email_" + $(Get-Date).ToString("yymmddHHmmssff") +  ".txt" 
#Note: Handle $Test carefully if set to false emails will go to email-list from database
$Test= $false;
$WaveVMID = 0;

$Query = “select V.WaveVMID, W.WaveName, Upper(M.Server) as Server, M.Percentage , MigState, MigStart, MigEndTime, dbo.fnWaveVMEmailData(v.WaveVMID ) as 'Email'
, V.IPAddress,V.OS, V.Sys_Owner , V.BEATID , V.Apps 
...
and rtrim(ltrim(M.[Percentage])) ='100' and ltrim(rtrim(m.MigState)) = 'MIGRATED' and w.WaveName = ltrim(rtrim('" + $Wave + "')) "
$Query
    ...
    ,"guilherme.viggiani@bayer.com"
        ...
        foreach ($recipient in $to) {
            try {
                ...
                $Output= "Email-$Counter server $($Row.Server.Trim()) Migration email sent successfully to $recipient @ $(Get-Date)"
                Write-Output $Output
                $Output | Out-File -FilePath $WaveOutputFilPath -Append 
                $listA | Out-File -FilePath $WaveOutputFilPath -Append 
                 if (!$Test)  {
                        $Query =  "Update tblWaveVM set [MigrationStatusId] = 7 ,[MigrationCompleteDate] = getDate() where WaveVMID = $WaveVMID ;"
