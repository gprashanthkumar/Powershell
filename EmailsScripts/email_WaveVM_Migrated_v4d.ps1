...
$MobilityGroupSet = "Webl_WAVE27w_USCTL_MIG_RAV__SEQ09"
$Group = 3
...
$Query = “select distinct V.loc, V.appgroup, V.WaveVMID, W.WaveName, Upper(M.Server) as Server, M.Percentage , MigState, MigStart, MigEndTime,
 dbo.fnWaveVMEmailData(v.WaveVMID ) as 'Email', dbo.fnWaveVMBOSO_EmailData(v.WaveVMID ) as 'BOSOEmails'
...
where (M.Batchid = (select Max(BatchID) from  [dbo].[tblHCXMigation_status_data] where dbo.fnTrimandEmptyvalue(LOC) = 'DE' )
or 
M.Batchid = (select Max(BatchID) from  [dbo].[tblHCXMigation_status_data] where dbo.fnTrimandEmptyvalue(LOC) = 'US' ))
...
$Query += "and ISNULL(V.Apps,'N/A')  not like '%Citrix%'"
$Query += "and Upper(M.Server)  in   ( 'USD11-CCPFBDPA1','USVA3-CCPFBDPA2' )  "

}elseif ($Group -eq 2) {
$Query += "and ISNULL(V.Apps,'N/A')  not like '%Citrix%'"
$Query += "and Upper(M.Server) not in ('BY-SDRAMIG2') "
...
$Query += "and ISNULL(V.Apps,'N/A')  like '%Citrix%'"
$Query += "and Upper(M.Server)  in ('BYDE276PMEI', 'BYDE276W4BQ' ) "
}
elseif ($Group -eq 5) {

$Query += "and ISNULL(V.Apps,'N/A')  like '%Citrix%'"
$Query += "and Upper(M.Server) not in ('BYUS0080YD2' ,'BYUS0086PZ9') "
}
elseif ($Group -eq 6) {

$Query += "and isNull(V.MobilityGroupSet,'') in  ('" + $MobilityGroupSet + "')"
