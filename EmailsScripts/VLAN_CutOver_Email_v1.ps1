...
$WaveOutputFilPath = "C:\Temp\VLANCutOver\$Wavefolder\VLANCutover_notification_email_" + $(Get-Date).ToString("yymmddHHmmssff") +  ".txt" 
...
where c.CutOverName = 'VC-1089 and 1910' "
$Query
...
if ( $firstrow ) {
    ...
    ,"analia.monzon@bayer.com"
        ...
        $subject = "Planned VLAN Cutover Activity Impacting Server(##Server##) -Application (##Apps##) Downtime Notification"
        $body = "This is a test email to check SMTP functionality from AVS migration project VM."
        $templatePath = "C:\Temp\email\email_cutover_Notify_v1.html"
        ...
        foreach ($recipient in $to) {
            try {
                ...
                $Output= "Email-$Counter server $($Row.Server.Trim()) impact due to vlan cutover impact email sent successfully to $to @ $(Get-Date)"
                Write-Output $Output
                $Output | Out-File -FilePath $WaveOutputFilPath -Append 
                $listA | Out-File -FilePath $WaveOutputFilPath -Append 
                 if (!$Test)  {
                        $Query =  "Update tblVLANCutOver_VMs set [notification] = getDate() where VMID = $WaveVMID ;"
