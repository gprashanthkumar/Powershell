...
$SleepSeconds = 2;
$Output = "";
$WaveOutputFilPath = "C:\Temp\$Wavefolder\OCI_VMMigrate_start_email_" + $(Get-Date).ToString("yymmddHHmmssff") +  ".txt" 

$firstrow = $true;
$WaveVMID = 0;

$Query = “select distinct  v.loc, V.appgroup, V.WaveVMID, W.WaveName, Upper(v.VMName) as Server, dbo.fnWaveVMEmailData(v.WaveVMID ) as 'Email', dbo.fnWaveVMBOSO_EmailData(v.WaveVMID ) as 'BOSOEmails', V.IPAddress,V.OS, isnull(V.Sys_Owner,'N/A') as Sys_Owner , isnull(V.BEATID,'N/A') as BEATID ,
...
where Upper(v.VMName) not in ('BY-UC4P', 'BYDE005LJ9C') and   W.WaveName in (SELECT value FROM STRING_SPLIT('" + $Waves + "', ','))"
$Query
...
if( $rowCount.Count -gt 0) {
 $Row.WaveName +"`n"  | Out-File -FilePath $WaveOutputFilPath -Append 
    ...
    ,"analia.monzon@bayer.com"
        ...
        $subject = "CCP-Azure native Migration ##Wave##: ##Server## (Apps: ##Apps##) Pre-Notification. "
        $body = "This is a test email to check SMTP functionality from AVS migration project VM."
        $templatePath = "C:\Temp\OCI\Emails\OCI_email_WaveVM_PreWeek_notify_v1.html"
